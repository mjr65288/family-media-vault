...
- **shadcn/ui's default `Button` sizes are below the 44px mobile touch-target minimum** (`default`/`lg`/`icon-lg` all top out at `h-9`/`size-9` = 36px). Every interactive control on an authenticated page overrides this with an explicit `h-11` (44px) or `h-12` (48px, used for the primary auth-form CTAs) className — see `MobileNav.tsx`'s hamburger (`h-11 w-11`) or any `DeleteFamilyButton`/`DeleteAlbumButton`. Don't drop a bare `<Button>` onto a page without checking its rendered height.
- **`npx shadcn` CLI flags changed from what you might remember**: no more `style`/`baseColor`, now `--preset` (`nova`/`vega`/`maia`/`lyra`/`mira`/`luma`/`sera`/`rhea`) and `--base` (`base`/`radix`). This project uses `--base radix` — Sheet and AlertDialog need Radix's focus-trap/ARIA guarantees. Re-running `npx shadcn init` without these flags may prompt differently than expected.
- **Leftover `next dev` processes silently accumulate in-memory rate-limit state** (`lib/rate-limit.ts`) across your testing session, since bash's `kill %1` only kills the shell job handle, not the actual Next.js process tree. If you get an unexpected 429 while manually testing, check `netstat -ano | grep :3000` (or `lsof -i :3000` on macOS/Linux) for a stale process before assuming the rate limiter itself is broken.
...
| shadcn/ui primitives | `components/ui/` (Button, Input, Label, Card, Sheet, AlertDialog — added via `npx shadcn add <name>`, config in `components.json`) |
| Authenticated nav shell | `components/AuthenticatedShell.tsx` (header + horizontal nav, wraps dashboard/album pages only — not `/`, `/login`, `/register`), `components/MobileNav.tsx` (hamburger + Sheet, the only client piece of the shell) |
